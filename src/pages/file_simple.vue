  ...
  <div class="zfile-index-body-wrapper" @contextmenu="showFileMenu">
    <div class="zfile-index-body"
      ...
      <el-dialog draggable custom-class="zfile-office-dialog"
                 :title="fileDataStore.currentClickRow.name"
                 v-model="dialogOfficeVisible" @close="closeOfficeDialog">
        <OfficeViewer
...
// 文件类别数据相关
import useFileData from "~/composables/file/useFileDataSimple";
// 直链相关
...
// 文件编辑缓存
import { cacheEditingFile } from "~/api/only-office";
...
const closeOfficeDialog = () => {
	...
	param.fileName = fileDataStore.currentClickRow.name;
	cacheEditingFile(param);
}
